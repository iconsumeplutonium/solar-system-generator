...
public class PlanetRing : MonoBehaviour {
	...
	public void SetUpRing() {
		...
		ring.transform.localRotation = (transform.rotation.z <= 180) ? Quaternion.Euler(-3.3f, 0f, transform.rotation.z) : Quaternion.Euler(-7.69f, 0f, transform.rotation.z);
	...
	public void BuildRingMesh(float innerRadius, float thickness) {
		//build ring mesh
